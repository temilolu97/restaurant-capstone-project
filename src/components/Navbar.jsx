      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          {navItems.map((navItem) => (
            <div
              key={navItem.item}
              onClick={() => handleClick(navItem.item)}
              className={`flex items-center  rounded-full px-[34px] py-[9px] font-semibold hover:bg-[#fc8a06] hover:text-white ${
                activeItem == navItem.item
                  ? "border bg-[#fc8a06] text-white"
                  : ""
              }`}
            >
              {navItem.item}
